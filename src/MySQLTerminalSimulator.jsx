...
function ToolbarButton({ icon:Icon, label, onClick }){
  return (
    <button onClick={onClick} className="flex items-center gap-2 px-3 py-2 rounded-2xl shadow-sm border hover:shadow-md transition text-sm">
...
export default function MySQLTerminalSimulator(){
  ...
  const outputContainerRef = useRef(null); // Add ref for output container
  ...
  function exportJSON(){
    ...
    a.href = url; a.download = 'mysql-sim-export.json'; a.click();
    ...
    inp.onchange = () => {
      ...
      reader.onload = () => {
        try {
          ...
          setEngine(obj); appendOut(`[${now()}] Imported database JSON.`);
        } catch (e){ appendOut('ERROR: Failed to import JSON.'); }
      };
      ...
      <div className="max-w-5xl mx-auto space-y-4">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold">MySQL Web Terminal Simulator</h1>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <ToolbarButton icon={Eraser} label="Clear Screen" onClick={()=> setOutput([])} />
            <ToolbarButton icon={Download} label="Export JSON" onClick={exportJSON} />
            <ToolbarButton icon={Upload} label="Import JSON" onClick={importJSON} />
        ...
        <div className="grid grid-cols-1 gap-4">
          <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="bg-black/50 rounded-2xl shadow-xl border border-white/10 p-4">
            <div className="font-mono text-xs text-slate-400 mb-2">Type SQL and press Enter (Shift+Enter for newline). Commands end with <span className="text-slate-200">;</span> or <span className="text-slate-200">\\G</span>. Ctrl+L to clear.</div>
            <div ref={outputContainerRef} className="bg-black rounded-xl p-3 h-[300px] sm:h-[420px] overflow-auto border border-white/10">
              <pre className="whitespace-pre-wrap text-sm leading-relaxed">{output.join('\n')}</pre>
            </div>
            <div className="mt-3 flex items-start gap-2">
              <span className="font-mono mt-3">mysql&gt;</span>
              <textarea
                ref={inputRef}
                value={buffer}
                ...
                className="flex-1 bg-black/70 border border-white/10 rounded-xl p-3 font-mono text-sm min-h-[56px] focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              <button onClick={onEnter} className="px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 shadow h-fit self-end">
          ...
          <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="bg-white/5 rounded-2xl shadow-xl border border-white/10 p-4 space-y-4">
            ...
            <ol className="list-decimal list-inside space-y-1 text-slate-200 text-sm">
              <li>Click <b>Seed Sample</b> (auto-creates DB <code>school</code> with <code>students</code> table).</li>
              <li>Try: <code className="bg-black/40 px-1 rounded">SELECT * FROM students;</code></li>
              <li>Insert a row: <code className="bg-black/40 px-1 rounded">INSERT INTO students (first_name,last_name,age,course) VALUES ('Ranz','Gutierrez',20,'BSIT');</code></li>
            ...
            <h3 className="font-semibold mt-4">Supported MySQL-ish Meta Commands</h3>
...
CREATE DATABASE school;\nUSE school;\nCREATE TABLE students (\n  student_id INT PRIMARY KEY AUTO_INCREMENT,\n  first_name VARCHAR(50),\n  last_name VARCHAR(50),\n  age INT,\n  course VARCHAR(50)\n);\nINSERT INTO students (first_name,last_name,age,course) VALUES\n  ('Mark','Lendio',21,'BSIT'),\n  ('Anna','Lopez',20,'BSCS');\nSELECT * FROM students ORDER BY age DESC LIMIT 5;\nSELECT * FROM students WHERE course='BSIT'\n  ORDER BY first_name ASC\n  LIMIT 10 OFFSET 0;\nSELECT COUNT(*) FROM students;\nDESCRIBE students;\n`}
            ...
            <p className="text-xs text-slate-400">Note: This is a learning simulator. Syntax coverage is practical but not perfect; real MySQL is stricter and has more features.</p>
          </motion.div>
        </div>
      </div>
    </div>
